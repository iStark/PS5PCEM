...
- Asterix & Obelix: Slap Them All! now completes its Unity and PSN plug-in
  ...
  then 240–250 ms, to typically 46–49 ms per frame. The final Unity compositor
  also retains the guest negative-height viewport as scanout orientation, so
  the scene and UI are no longer vertically inverted. A 2,760-flip validation
  run remained submission-clean and reproduced the gameplay shown below.
...
| **Asterix & Obelix: Slap Them All!** | Maps the Unity/PSN plug-in graph, passes GameUpdate, trophy, entitlement, WebApi, and player-review bootstrap calls, accepts four-byte-aligned AGC shader headers, and reaches repeatable 1920×1080 gameplay. SceAvPlayer returns the ATL intro as correctly decoded NV12 frames; the translated guest pixel shader converts the staged planes on the GPU, resident fullscreen copies avoid the former GPU→CPU→GPU round trip, and synchronous AGC retirement removes the three-second Unity polling timeout. Equivalent completion edges are coalesced and paced by one display interval instead of the former fixed 250 ms delay. Scanout orientation follows the final compositor's negative-height viewport, keeping gameplay and UI upright. The latest 2,760-flip run remained submission-clean and reproduced 48-draw gameplay at typically 46–49 ms per frame on the current RTX 3070 Ti host | Gameplay is verified through the opening forest scene. The renderer still submits and fence-waits almost every draw separately; batching command buffers and descriptor state is the primary remaining performance target, followed by broader input/audio coverage and longer play-session stability |
