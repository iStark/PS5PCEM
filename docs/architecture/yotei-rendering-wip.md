...
Archived investigation record. The rendering source described here was
integrated into `main` at `4e9453e`; the GPU-page mapping fix from
`perf/yotei-1fps` was integrated at `b9231ec`. Both source branches are now
merged into `main`, where development continues. Current behavior and opt-in
controls are documented in [the Vulkan architecture](vulkan.md). Run details
and validation limits below describe the original snapshot, not the latest run.

This snapshot preserves the four outstanding source files reviewed against
...
## Run identified when the snapshot was recorded
...
## Checks performed on the original snapshot
