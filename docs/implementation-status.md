...
[← Documentation index](README.md) · [Project status and compatibility](project-status.md) · [Project README](../README.md)

What the emulator can currently do, subsystem by subsystem. The observed title
milestones and development captures live in
[Project status and compatibility](project-status.md); this page is the detail
behind them.
...
- Titles that decode video through `libSceVideodec2` rather than SceAvPlayer are
  supported as well. H.264 access units are decoded by the host, converted from
  NV12 with BT.709 coefficients, and presented directly, and decoding is paced
  to about one picture per display interval so a title feeding access units as
  fast as they are accepted does not drain its whole movie in a few frames. A
  decoder reports the frame-slot size a title divides its arena by, so it
  reserves slots a decoded picture fits in.
