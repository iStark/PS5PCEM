...
| **Cat Quest III** | Enters the Unity graphics loop and presents the animated 3840×2160 illustrated intro with upright artwork, subtitles, and Skip prompt. Procedural full-screen triangles retain their translated vertex shader and runtime UV scale/bias; the resident-copy shortcut previously inferred an incorrect flip from viewport sign alone. Eight Vulkan cases cover both viewport signs, both UV orientations, and guest-memory/resident sources | Validation covers startup and intro orientation; gameplay, broader input/audio behavior, and longer-run stability are not claimed. Baseline runs also encountered guest-worker memory faults outside the orientation fix |
