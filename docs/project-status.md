...
| **Quake II (2023)**<br><img src="images/quake-ii-title.png" width="240" alt="Quake II title screen rendered by PS5PCEM"> | Links its complete module graph, mounts `pak0.pak`, and runs the engine into a level, but the presented image stops updating at the photosensitivity warning | Reaching this point on September 16, 2026 needed 26 firmware imports the emulator did not provide, across five libraries; `libSceVoice`, `libSceSystemGesture` and `libSceLoginDialog` did not exist at all. `sceKernelGetdents` also returned one entry per call padded to a fixed 512-byte record, and this title reads `/app0/baseq2` exactly once: it saw a directory holding nothing but `.` and missed the 1.7 GiB `pak0.pak` beside it. With packed BSD dirent records it adds that pack file with 14,663 files, registers 5 episodes and 232 maps, loads the Tutorial level, writes and reads save data, initialises audio, and submits 991 VideoOut flips; controller presses reach `scePadReadState` as clean edges. The picture nevertheless freezes: colour writes are honoured only while `CB_COLOR_CONTROL.MODE` is NORMAL, and from about flip 320 every draw in the frame reports DISABLE while still naming a non-zero `CB_TARGET_MASK`, so the frame's colour is discarded and `color_export_generation` stops advancing. A traced frame at flip 60, while the picture still updated, draws into the scanout with MODE=NORMAL and a `0xf` write mask; the same trace at flip 1200 shows the five-target deferred G-buffer and the scanout draws with MODE=DISABLE and a zero mask. Suppressing those draws is what keeps Ghost of Yōtei's G-buffer pixel shader from exporting zeros over its scene, so the two titles need a signal that separates them. Gameplay is not claimed. [Title capture](images/quake-ii-title.png) |
