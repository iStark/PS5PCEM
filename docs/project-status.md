...
| **Big Helmet Heroes**<br><img src="images/big-helmet-heroes-menu.png" width="240" alt="Big Helmet Heroes main menu with readable options and an incorrectly rendered background"> | **Intro playback · main menu reached · graphical errors · gameplay unverified** | Verified on September 18, 2026: the opening movie leads to the title logo and readable `NEW GAME` / `OPTIONS` menu. Correcting private AGC submission retirement removes the post-intro wait; matching vertex attributes by the shader's scalar offset restores UI positions. The 3D background still has severe rendering errors. Corrected NAPS mapping and Kraken decoding restore all ten UE PAK archives. [Menu capture](images/big-helmet-heroes-menu.png) · [Intro capture](images/big-helmet-heroes-intro.png) |
...
![Big Helmet Heroes main menu with rendering errors](images/big-helmet-heroes-menu.png)

*The post-intro menu now displays the title logo, NEW GAME, OPTIONS and the
version number. This is a capture of the emulator's actual presentation output;
the purple/blue background is a remaining rendering error. Gameplay has not
been verified.*
