...
pub fn formatOperand(op: Operand, w: *Writer) Writer.Error!void {
    ...
    switch (op.kind) {
        ...
        .ttmp => try w.print("ttmp{d}", .{op.reg}),
        .flat_scratch_base_lo => try w.writeAll("src_flat_scratch_base_lo"),
        .flat_scratch_base_hi => try w.writeAll("src_flat_scratch_base_hi"),
        .shared_base => try w.writeAll("src_shared_base"),
        .shared_limit => try w.writeAll("src_shared_limit"),
        .private_base => try w.writeAll("src_private_base"),
        .private_limit => try w.writeAll("src_private_limit"),
        .lds_direct => try w.writeAll("src_lds_direct"),
