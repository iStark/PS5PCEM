...
pub const GuestMemory = struct {
    ...
    track_gpu_read: ?*const fn (?*anyopaque, u64, usize) u64 = null,
...
const InstanceFunctions = struct {
    ...
    get_physical_device_features_2: vk.PfnGetPhysicalDeviceFeatures2,
    ...
    fn load(loader: *const Loader, instance_handle: vk.Instance) Error!InstanceFunctions {
        return .{
            ...
            .get_physical_device_features_2 = try loader.instance(instance_handle, vk.PfnGetPhysicalDeviceFeatures2, "vkGetPhysicalDeviceFeatures2"),
...
const DeviceFunctions = struct {
    ...
    create_semaphore: vk.PfnCreateSemaphore,
    destroy_semaphore: vk.PfnDestroySemaphore,
    get_semaphore_counter_value: vk.PfnGetSemaphoreCounterValue,
    wait_semaphores: vk.PfnWaitSemaphores,
    ...
    fn load(get_proc: vk.PfnGetDeviceProcAddr, device: vk.Device) Error!DeviceFunctions {
        return .{
            ...
            .create_semaphore = try deviceProc(get_proc, device, vk.PfnCreateSemaphore, "vkCreateSemaphore"),
            .destroy_semaphore = try deviceProc(get_proc, device, vk.PfnDestroySemaphore, "vkDestroySemaphore"),
            .get_semaphore_counter_value = try deviceProc(get_proc, device, vk.PfnGetSemaphoreCounterValue, "vkGetSemaphoreCounterValue"),
            .wait_semaphores = try deviceProc(get_proc, device, vk.PfnWaitSemaphores, "vkWaitSemaphores"),
...
const DrawUploadCacheEntry = struct {
    guest_address: u64,
    size: usize,
...
const DeferredVulkanObject = union(enum) {
    ...
    storage_image_release: usize,
};

const DeferredVulkanObjectEntry = struct {
    retire_tick: u64,
    object: DeferredVulkanObject,
};

const command_buffer_pending_tick = std.math.maxInt(u64);
...
fn reserveAlignedRange(cursor: *usize, capacity: usize, bytes: usize, alignment: usize) ?usize {
    if (bytes == 0 or bytes > capacity) return null;
    const offset = std.mem.alignForward(usize, cursor.*, alignment);
    const end = std.math.add(usize, offset, bytes) catch return null;
...
// live in one frame (Tetris Effect reaches 42 before its second flip). Keeping
// only 16 forced GPU -> CPU -> tiled guest-memory -> GPU round trips while an
...
/// Prospero RECT_LIST auto draws describe one logical rectangle with counts
/// 1/3/4. The NGG/export shader uses the system vertex index to synthesize the
/// missing corner, so the host must actually invoke it four times. Completing
/// a fourth corner after the VS loses guest-computed position and UV logic.
fn expandRectangleListDraw(primitive_type: u32, draw: GuestDraw) GuestDraw {
    if (draw.index_count != null) return draw;
    var expanded = draw;
    if (primitive_type == 7 and
        (draw.vertex_count == 1 or draw.vertex_count == 3 or draw.vertex_count == 4))
    {
        expanded.vertex_count = 4;
    } else if (primitive_type == 17 and draw.vertex_count == 3) {
        expanded.vertex_count = 4;
    }
    return expanded;
}

fn rectangleCompletionMode(
    vertex_stage: gpu.resources.ShaderStage,
    primitive_type: u32,
    draw: GuestDraw,
    fullscreen_corner: bool,
) u32 {
    // Unity's merged NGG path sometimes submits three indexed corners as an
    // ordinary TRI_LIST even though the hardware is expected to complete the
    // rectangle. The resource-backed corner check is deliberately performed
    // by the caller; mode 2 completes the corner opposite v0.
    if (fullscreen_corner and vertex_stage == .export_shader and
        primitive_type == 4 and draw.index_count == 3)
    {
        return 2;
    }
    if (primitive_type == 7 or primitive_type == 17) {
        return @intFromBool(draw.index_count == null and draw.vertex_count == 3);
    }
    return 0;
}
...
fn colorTargetFormat(descriptor: gpu.resources.ColorTarget) ?ColorTargetFormat {
    return switch (descriptor.format) {
        ...
        14 => if (descriptor.number_type == 7)
            .{ .vulkan = vk.format_r32g32b32a32_sfloat, .bytes_per_texel = 16 }
        else
            null,
        else => null,
    };
...
const color_export_identity: u8 = 0xe4;
...
fn colorTargetExportMapping(descriptor: gpu.resources.ColorTarget) u8 {
    ...
    const mappings = [4][4]u8{
        .{ 0xe4, 0xe4, 0xe4, 0xe4 },
        .{ 0xe1, 0x6c, 0xb4, 0xc6 },
        .{ 0xc6, 0xe1, 0xc6, 0x1b },
        .{ 0x27, 0x63, 0x87, 0x93 },
    };
    return mappings[descriptor.component_swap][component_count - 1];
}

fn mapColorWriteMask(logical_mask: u8, mapping: u8) u32 {
    var physical_mask: u32 = 0;
    for (0..4) |physical| {
...
fn hostColorTargetDescriptor(descriptor: gpu.resources.ColorTarget) gpu.resources.ColorTarget {
    ...
    // rendering/geometry-stage exports are exposed, retain the guest base
    // slice but narrow an array view to that slice instead of rejecting the
    // whole draw. This keeps Unity's small RGBA16F luminance/bloom MRT passes
    // alive and preserves the correct physical source offset on readback.
    if (host.last_array_slice > host.base_array_slice) {
        host.last_array_slice = host.base_array_slice;
    }
    return host;
}
...
const maximum_texture_probes = 256;
...
fn traceShaderAnalysisFailure(
    ...
    const header = header_address orelse {
        std.debug.print(
            "[vulkan dcb] shader analysis failed stage={s} program=0x{x} header=missing error={s}\n",
            .{ @tagName(stage), program_address, @errorName(err) },
        );
        return;
    };
    const code_address = reader.readU64(header + 0x10) catch 0;
    const header_size = reader.readU32(header + 0x40) catch 0;
    const shader_size = reader.readU32(header + 0x44) catch 0;
    const trailing = reader.readU32(header + 0x58) catch 0;
    std.debug.print(
        "[vulkan dcb] shader analysis failed stage={s} program=0x{x} header=0x{x} code=0x{x} header_size=0x{x} shader_size=0x{x} type={d} sh_regs={d} error={s}\n",
        .{
            @tagName(stage),
            program_address,
            header,
...
fn shaderProgramByteLimit(
    reader: gpu.ShaderMemoryReader,
    header_address: ?u64,
    program_address: u64,
) ?usize {
    const header = header_address orelse return null;
    const code_address = reader.readU64(header + 0x10) catch return null;
    const shader_size = reader.readU32(header + 0x44) catch return null;
    if (program_address < code_address) return null;
    const program_offset = program_address - code_address;
    if (program_offset >= shader_size) return null;
    const remaining = @as(u64, shader_size) - program_offset;
    if (remaining < @sizeOf(u32) or remaining > maximum_shader_code_bytes) return null;
...
const FrameProfile = struct {
    ...
    sampled_lookup_ns: u64 = 0,
    texture_probe_ns: u64 = 0,
    render_target_create_ns: u64 = 0,
    depth_target_create_ns: u64 = 0,
...
const ComputeResources = struct {
    ...
    fn deinit(self: *ComputeResources, renderer: *Renderer) void {
        ...
        // views and samplers retire after the frame batch fence signals.
...
fn isFullscreenCornerTriangle(
    reader: gpu.ShaderMemoryReader,
    resources: *const ComputeResources,
    draw: GuestDraw,
) bool {
    if (draw.index_count == null or draw.index_count.? != 3 or draw.index_address == 0) return false;

    var indices: [3]u32 = undefined;
    const index_stride: u64 = if (draw.index_uint32) 4 else 2;
    for (&indices, 0..) |*index, element| {
        const index_address = draw.index_address + @as(u64, @intCast(element)) * index_stride;
        index.* = if (draw.index_uint32)
            reader.readU32(index_address) catch return false
        else
            reader.readU16(index_address) catch return false;
    }

    for (resources.mappings[0..resources.mapping_count]) |mapping| {
        if (!mapping.use_vertex_index or mapping.unified_format != 77 or mapping.stride < 16) continue;
        const source_offset = mapping.soffset_value orelse 0;
        if (source_offset != 0) continue;
        const slot: usize = @intCast(mapping.descriptor_index);
        var corners: [3][4]f32 = undefined;
        var valid = true;
        for (indices, 0..) |index, corner| {
            const byte_offset = std.math.mul(u64, index, mapping.stride) catch {
                valid = false;
                break;
            };
            if (byte_offset + 16 > @as(u64, @intCast(resources.sizes[slot]))) {
                valid = false;
                break;
            }
            var words: [4]u32 = undefined;
            reader.readWords(resources.addresses[slot] + byte_offset, &words) catch {
                valid = false;
                break;
            };
            for (&corners[corner], words) |*component, word| component.* = @bitCast(word);
        }
        if (!valid) continue;

        for (corners) |corner| {
            if (@abs(@abs(corner[0]) - 1.0) > 0.0001 or
                @abs(@abs(corner[1]) - 1.0) > 0.0001 or
                @abs(corner[2]) > 0.0001 or @abs(corner[3] - 1.0) > 0.0001)
            {
                valid = false;
                break;
            }
        ...
        if (!valid) continue;
        // Mode 2 reconstructs v3 = v1 + v2 - v0. Require v0 to be the
        // shared corner and v1/v2 to be the diagonal pair so that equation is
        // exactly the missing fourth clip-space corner.
        const first_adjacent = corners[0][0] == corners[1][0] or corners[0][1] == corners[1][1];
        const second_adjacent = corners[0][0] == corners[2][0] or corners[0][1] == corners[2][1];
        const others_diagonal = corners[1][0] != corners[2][0] and corners[1][1] != corners[2][1];
        if (first_adjacent and second_adjacent and others_diagonal) return true;
    }
    return false;
}

const DrawVertexRange = struct {
    minimum: i64,
    maximum: i64,
};
...
fn drawVertexRange(reader: gpu.ShaderMemoryReader, draw: GuestDraw) ?DrawVertexRange {
    if (draw.index_count) |count| {
        ...
        return .{
            ...
            .maximum = @as(i64, maximum) + draw.vertex_offset,
        };
    }
    if (draw.vertex_count == 0) return null;
    return .{
        .minimum = draw.first_vertex,
        .maximum = @as(i64, draw.first_vertex) + draw.vertex_count - 1,
    };
}

fn validateVertexIndexMappings(
    reader: gpu.ShaderMemoryReader,
...
/// live V# uses the compact BufferFormat enum. Converting here lets a shader
/// fetch be paired with the right table entry even when fetch order differs
/// from semantic/location order.
fn vertexAttributeBufferFormat(format: u16) u16 {
    return switch (format) {
        0 => 0,
        4 => 1,
        8 => 2,
...
fn sampledImageDimensionForInstruction(
    ...
    // GFX10's DIM selects the coordinate payload while the T# resource type is
    // the authoritative view kind. Unity uses three-coordinate MIMG forms for
    // both volumes and ordinary 2D render targets; Vulkan requires the view
    // dimension to match the descriptor, so refine it after materializing T#.
    switch (image_type) {
        .color_3d => return .three_d,
        .color_2d_array => return .two_d_array,
        .color_1d, .color_2d => return .two_d,
        else => {},
    }
...
const CachedResidentSampler = struct {
    descriptor: gpu.resources.SamplerDescriptor,
    sampler: vk.Sampler,
};

fn aliasRange(address: u64, byte_count: anytype) image_alias.Range {
    return .{ .address = address, .size = @intCast(byte_count) };
}

fn colorTargetAliasSignature(target: GuestColorTarget) image_alias.Signature {
    return .{
...
fn depthTargetAliasSignature(target: GuestDepthTarget) image_alias.Signature {
    return .{
        .format = target.format,
        .width = target.width,
        .height = target.height,
        .tile_mode = @intFromEnum(target.tile_mode),
        .samples_log2 = target.samples_log2,
        .aspect = if (target.has_stencil) .depth_stencil else .depth,
    };
}

fn descriptorAliasSignature(
    descriptor: gpu.ImageDescriptor,
    image_format: u32,
    mip_levels: u8,
) image_alias.Signature {
    return .{
        .format = image_format,
        .width = descriptor.width,
        .height = descriptor.height,
        .depth = if (descriptor.image_type == .color_3d) descriptor.depth_or_layers else 1,
        .mip_levels = mip_levels,
        .layers = if (descriptor.image_type == .color_3d)
            1
        else
            @intCast(@min(descriptor.depth_or_layers, std.math.maxInt(u16))),
        .tile_mode = @intFromEnum(descriptor.tile_mode),
        .samples_log2 = descriptor.samplesLog2(),
    };
}
...
pub const Renderer = struct {
    ...
    descriptor_set: vk.DescriptorSet,
    descriptor_cursor: usize = 0,
    descriptor_slot_ticks: [maximum_frame_descriptor_sets]u64 = @splat(0),
    pending_descriptor_slots: std.ArrayList(usize) = .empty,
    current_descriptor_slot: ?usize = null,
    dynamic_scalar_buffer: ?OwnedBuffer = null,
    dynamic_scalar_mapping_base: ?[*]u32 = null,
    ...
    frame_command_buffers: std.ArrayList(vk.CommandBuffer) = .empty,
    command_buffer_ticks: std.ArrayList(u64) = .empty,
    ...
    pub fn init(allocator: std.mem.Allocator, options: Options) (Error || std.mem.Allocator.Error)!Renderer {
        ...
        var timeline_enable = vk.PhysicalDeviceTimelineSemaphoreFeatures{
            .p_next = if (descriptor_partially_bound) &descriptor_indexing_enable else null,
        ...
        const semaphore_info = vk.SemaphoreCreateInfo{ .p_next = &semaphore_type };
        var timeline_semaphore: vk.Semaphore = 0;
        if (device_functions.create_semaphore(device, &semaphore_info, null, &timeline_semaphore) != vk.success) {
            return Error.TimelineSemaphoreCreationFailed;
        }
        errdefer device_functions.destroy_semaphore(device, timeline_semaphore, null);
        ...
        const descriptor_layout_info = vk.DescriptorSetLayoutCreateInfo{
            .p_next = if (descriptor_partially_bound) &descriptor_binding_flags_info else null,
        ...
        const storage_image_pool_size = vk.DescriptorPoolSize{
            .descriptor_type = vk.descriptor_type_storage_image,
            .descriptor_count = maximum_storage_images * maximum_frame_descriptor_sets,
        };
        ...
        const descriptor_pool_info = vk.DescriptorPoolCreateInfo{
            .max_sets = maximum_frame_descriptor_sets,
        ...
        const descriptor_sets = try allocator.alloc(vk.DescriptorSet, maximum_frame_descriptor_sets);
        errdefer allocator.free(descriptor_sets);
        var descriptor_layouts: [maximum_frame_descriptor_sets]vk.DescriptorSetLayout =
            @splat(descriptor_set_layout);
        const descriptor_allocate_info = vk.DescriptorSetAllocateInfo{
            .descriptor_pool = descriptor_pool,
            .descriptor_set_count = maximum_frame_descriptor_sets,
            .set_layouts = &descriptor_layouts,
        };
        if (device_functions.allocate_descriptor_sets(device, &descriptor_allocate_info, descriptor_sets.ptr) != vk.success) {
        ...
        for (descriptor_sets, 0..) |set, slot| {
            const scalar_buffer_info = vk.DescriptorBufferInfo{
                ...
                .offset = slot * descriptor_scalar_stride,
    ...
    pub fn deinit(self: *Renderer) void {
        ...
        self.color_passes = .empty;
        for (self.resident_image_views.items) |cached| {
            self.device_functions.destroy_image_view(self.device, cached.view, null);
        }
        self.resident_image_views.deinit(self.allocator);
        self.resident_image_views = .empty;
        for (self.resident_samplers.items) |cached| {
            self.device_functions.destroy_sampler(self.device, cached.sampler, null);
        }
        self.resident_samplers.deinit(self.allocator);
        ...
        self.frame_command_buffers.deinit(self.allocator);
        self.command_buffer_ticks.deinit(self.allocator);
        self.pending_command_buffers.deinit(self.allocator);
        self.pending_command_slots.deinit(self.allocator);
        self.pending_descriptor_slots.deinit(self.allocator);
        self.deferred_vulkan_objects.deinit(self.allocator);
    ...
    pub fn stageGuestStorageBufferAt(
        ...
        if (!entry.gpu_dirty) {
            ...
            if (tracked_generation != 0 and (!self.draw_uploads_enabled or cache_hit)) {
                if (entry.page_generation != tracked_generation) {
                    ...
                    while (true) : (attempt += 1) {
                        if (!memory.read(memory.context, guest_address, destination[0..size])) {
                        ...
                        const after = if (memory.gpu_generation) |generation|
                            ...
                            observed_generation;
                        if (after == observed_generation) {
                            entry.page_generation = observed_generation;
                            break;
                        }
                        if (attempt >= 1) {
                            // A continuously written range is still usable as a
                            // snapshot, but do not cache it as current.
                            entry.page_generation = 0;
                            break;
                        }
                        observed_generation = if (memory.track_gpu_read) |track|
                            track(memory.context, guest_address, size)
                        else
                            0;
            ...
            if (self.draw_uploads_enabled) {
                ...
                self.frame_profile.storage_upload_bytes +%= size;
                self.buffer_uploads += 1;
                try self.draw_upload_cache.append(self.allocator, .{
                    .guest_address = guest_address,
                    .size = size,
                    .upload = upload,
                });
    ...
    pub fn dispatchRdna2State(
        ...
        if (try self.tryEmulateSimpleBufferFill(
            memory,
            state,
            analysis,
            system_registers,
    ...
    fn tryEmulateSimpleBufferFill(
        ...
        if (inst.len != 4 or
            ...
            inst[2].opcode != .buffer_store_format_x or
            inst[3].opcode != .s_endpgm or
            local_size[0] != 64 or local_size[1] != 1 or local_size[2] != 1 or
            group_count[1] != 1 or group_count[2] != 1 or
            system.workgroup_id_sgprs[0] != 4 or
            system.workgroup_id_sgprs[1] != null or system.workgroup_id_sgprs[2] != null or
            system.local_invocation_id_components != 1 or
            !registerOperand(inst[0].dst, .vgpr, 0) or
            !registerOperand(inst[0].src0, .sgpr, 4) or
            inst[0].src1.kind != .integer_inline_constant or inst[0].src1.value != 6 or
            !registerOperand(inst[0].src2, .vgpr, 0) or
            !registerOperand(inst[1].dst, .vgpr, 1) or
            !registerOperand(inst[2].dst, .vgpr, 1) or
            !registerOperand(inst[2].src0, .vgpr, 0) or
            !registerOperand(inst[2].src1, .sgpr, 0) or
            inst[2].offset_enable or inst[2].memory_offset != 0)
        {
            return null;
        }
        const fill_value = switch (inst[1].src0.kind) {
            .integer_inline_constant, .literal_constant, .float_inline_constant, .null => inst[1].src0.value,
            else => return null,
        };
        const descriptor = try descriptorFromComputeUserData(state, 0);
        if (descriptor.address == 0 or descriptor.stride != 4 or descriptor.unified_format != 20 or
            descriptor.swizzle_enabled or descriptor.add_thread_id or
            descriptor.out_of_bounds_select != 0 or descriptor.dst_select[0] != 4)
        {
            return null;
        }
        const dispatched = std.math.mul(u64, group_count[0], local_size[0]) catch
            return Error.GuestBufferTooLarge;
        const records = @min(dispatched, @as(u64, descriptor.record_count));
        const byte_count_u64 = std.math.mul(u64, records, 4) catch return Error.GuestBufferTooLarge;
    ...
    fn prepareComputeResources(
        ...
        for (instructions) |inst| {
            ...
            const formatted_vertex_fetch = is_vertex_stage and inst.family != .smem and !is_store and switch (inst.opcode) {
                .buffer_load_format_x,
                .buffer_load_format_xy,
                .buffer_load_format_xyz,
                .buffer_load_format_xyzw,
            ...
            const resolved_descriptor = try resolveComputeBufferDescriptor(
                bindings,
                reader,
                analysis,
                &instruction_scalar,
            ...
            const vertex_attribute: ?gpu.VertexAttribute = if (resolved_descriptor) |resolved|
                ...
                if (vertex_table) |*table|
                    takeNextVertexAttribute(table, &used_vertex_attributes)
                else
                    null
            else
                null;
            const descriptor = resolved_descriptor orelse (if (vertex_attribute) |attribute|
            ...
            result.mappings[result.mapping_count] = .{
                ...
                // AGC's vertex table supplies the element separately and its
                // V# may leave DST_SEL at zero. Those zero selectors are not
                // an instruction to replace every fetched component with 0;
                // formatted vertex fetches use their natural XYZW order. The
                // recovered V# format remains authoritative because AGC's
                // attribute-format numbering is not uniformly identical to
                // the RDNA unified-format enum (values above 182 are common).
                .unified_format = descriptor.unified_format,
                .dst_select = if (vertex_attribute != null or formatted_vertex_fetch)
                    .{ 4, 5, 6, 7 }
                else
                    descriptor.dst_select,
        ...
        for (instructions) |inst| {
            ...
            const format = storageImageFormat(descriptor.unified_format) orelse {
                std.debug.print(
                    "[vulkan dcb] storage image pc=0x{x}: format {d} is unsupported addr=0x{x} {d}x{d}x{d} type={s} writable={any}\n",
                    .{
                        inst.pc,
                        descriptor.unified_format,
                        descriptor.address,
    ...
    fn destroyColorPass(self: *Renderer, pass: ColorPass) void {
        self.destroyFramebuffer(pass.framebuffer);
        self.destroyRenderPass(pass.render_pass);
    }
    ...
    /// Page tracking walks every guest page in a sampled allocation. A frame
    /// may bind the same 32 MiB texture hundreds of times, while guest writes
    ...
    fn sampledPageGeneration(self: *Renderer, memory: GuestMemory, address: u64, span: usize) u64 {
        ...
        const generation = if (memory.track_gpu_read) |track|
            track(memory.context, address, span)
        else
            0;
        const entry = TextureProbe{
            .address = address,
    ...
    fn invalidateTextureProbes(self: *Renderer, address: u64, span: usize) void {
        var index: usize = 0;
        while (index < self.texture_probe_count) {
            const probe = self.texture_probes[index];
            if (!probe.valid or !byteRangesOverlap(address, span, probe.address, probe.span)) {
                index += 1;
                continue;
            }
            self.texture_probe_count -= 1;
            self.texture_probes[index] = self.texture_probes[self.texture_probe_count];
            self.texture_probes[self.texture_probe_count] = .{};
        }
    }
    ...
    fn getGraphicsPipeline(
        ...
        if (self.graphics_pipelines.items.len < maximum_graphics_pipelines) {
            ...
            // A least-recently-used pipeline may still be named by this
            // frame's recorded commands. destroyPipeline therefore retires it
            // only after the batch fence while the cache slot is reused now.
            var oldest_index: usize = 0;
            ...
            self.destroyPipeline(victim.pipeline);
    ...
    /// Stage a color target without issuing one checked guest-memory read per
    /// texel. Large tiled 4K targets otherwise spend minutes crossing the
    /// MemoryReader callback roughly 8.3 million times before the first draw.
    fn stageColorTarget(
        self: *Renderer,
        target: GuestColorTarget,
        reader: gpu.ShaderMemoryReader,
        ...
        if (!memory.read(memory.context, target.descriptor.address, tiled))
            return Error.GuestMemoryReadFailed;
        try target.layout.detile(tiled, frame);
    }
    ...
    fn createCachedRenderTarget(self: *Renderer, target: GuestColorTarget) anyerror!CachedRenderTarget {
        ...
        const alias_token = try self.image_aliases.register(
            self.allocator,
            .color_target,
            aliasRange(target.descriptor.address, target.layout.required_source_bytes),
            colorTargetAliasSignature(target),
        );
        return .{
    ...
    fn destroyCachedRenderTarget(self: *Renderer, target: CachedRenderTarget) void {
        ...
        self.invalidateResidentImageViews(target.image.handle);
    ...
    fn destroyDepthPass(self: *Renderer, pass: ?DepthPass) void {
        ...
        self.destroyFramebuffer(active.framebuffer);
        self.destroyRenderPass(active.render_pass);
    }
    ...
    fn transitionRenderTargetToStorage(self: *Renderer, index: usize) anyerror!void {
        if (index >= self.render_targets.items.len) return Error.MissingPresentedFrame;
        const snapshot = self.render_targets.items[index];
        if (!snapshot.initialized) return Error.MissingPresentedFrame;
        const command_buffer = try self.beginOneShot();
        defer self.releaseOneShot(command_buffer);
        try self.transitionTrackedImage(
            command_buffer,
        ...
        try self.submitOneShot(command_buffer);
        // Reuse the existing non-attachment marker so the normal return path
        // transitions GENERAL back to COLOR_ATTACHMENT_OPTIMAL.
        self.render_targets.items[index].shader_read_layout = true;
    }
    ...
    fn stageResidentRenderTarget(
        ...
        const view = try self.residentImageView(
            cached.image.handle,
            if (dimension == .two_d_array) vk.image_view_type_2d_array else vk.image_view_type_2d,
            image_format,
            components,
    ...
    fn guestDepthTarget(descriptor: gpu.resources.DepthTarget) ?GuestDepthTarget {
        ...
        const depth_bytes_per_sample: u64 = if (descriptor.format == 1) 2 else 4;
        const texel_count = std.math.mul(u64, descriptor.width, descriptor.height) catch return null;
        const sample_count = @as(u64, 1) << @intCast(descriptor.samples_log2);
        const fallback_bytes = std.math.mul(
            u64,
            std.math.mul(u64, texel_count, depth_bytes_per_sample) catch return null,
            sample_count,
        ) catch return null;
        // Some legal MSAA register combinations are not importable by the
    ...
    fn acquireRenderTarget(self: *Renderer, target: GuestColorTarget) anyerror!usize {
        for (self.render_targets.items, 0..) |cached_snapshot, index| {
            ...
            // force that image through guest memory and upload it again. An
            // actual metadata write still invalidates the target through
            // prepareCmaskWrite/alias tracking.
            if (metadata_changed) cached.scanout_flip_vertical = false;
    ...
    fn materializeRenderTargetAt(self: *Renderer, address: u64) anyerror!bool {
        ...
        for (self.render_targets.items, 0..) |cached, index| {
            ...
            if (selected == null or cached.last_used_sequence > selected_sequence) {
                selected = index;
                selected_sequence = cached.last_used_sequence;
            }
        }
    ...
    fn blitResidentColorTarget(
        ...
        if (!initial_source.initialized or
            ...
            initial_source.target.descriptor.height != destination.descriptor.height)
        {
            return null;
        }
    ...
    fn drawPersistentGraphicsShaders(
        ...
        var index_upload: ?DrawUploadSlice = null;
    ...
    fn recordGuestColorTarget(self: *Renderer, target: GuestColorTarget, frame: []const u8) anyerror!void {
        ...
        for (self.completed_frames.items, 0..) |cached, index| {
            ...
            const previous = cached.target orelse continue;
            if (previous.descriptor.base_array_slice != target.descriptor.base_array_slice or
                previous.descriptor.last_array_slice != target.descriptor.last_array_slice or
                previous.descriptor.mip_level != target.descriptor.mip_level)
            {
                continue;
            }
            frame_index = index;
    ...
    /// This is also the explicit host-observation boundary used by standalone
    /// DCB tests whose synthetic streams do not contain RELEASE_MEM or a flip.
    pub fn flushPendingGuestWrites(self: *Renderer) anyerror!void {
        try self.finishDrawBatch();
        const memory = self.guest_memory orelse return Error.GuestMemoryUnavailable;
        var storage_image_index: usize = 0;
        while (storage_image_index < self.storage_image_cache.items.len) : (storage_image_index += 1) {
            try self.flushCachedStorageImage(memory, storage_image_index);
        }
        var storage_buffer_index: usize = 0;
        while (storage_buffer_index < self.guest_buffers.items.len) : (storage_buffer_index += 1) {
            try self.flushGuestStorageBuffer(storage_buffer_index);
        }
    ...
    /// Publishes dirty image writers whose allocation intersects a resource
    /// about to be reconstructed from guest memory. Exact base-address tests
    /// miss mip/subresource views and reinterpretations that start inside the
    ...
    fn drawGuestGraphics(
        ...
        // that stale address). Attaching and clearing it as depth rejects the
        // fullscreen compositor, leaving the exact scanout target black.
        ...
        if (ngg_lds_export_count != 0) {
            for (ngg_lds_exports[0..ngg_lds_export_count]) |ngg_export| {
                vertex_export_mask |= @as(u64, 1) << ngg_export.target;
                if (ngg_export.target >= 0x20) {
                    vertex_parameter_mask |= @as(u32, 1) << @intCast(ngg_export.target - 0x20);
                }
            }
        }
        ...
        for (0..fragment_input_controls.len) |attribute| {
            const attribute_bit = @as(u32, 1) << @intCast(attribute);
            if (fragment_attribute_mask & attribute_bit == 0) continue;
            const parameter: u5 = @truncate(fragment_input_controls[attribute]);
            if (vertex_parameter_mask & (@as(u32, 1) << parameter) != 0) {
        ...
        {
            std.debug.print(
                "[vulkan dcb] shader interface VS=0x{x} exports=0x{x} params=0x{x} ngg_lds={d} PS=0x{x} attrs=0x{x}->params=0x{x}\n",
                .{ vertex_address, vertex_export_mask, vertex_parameter_mask, ngg_lds_export_count, fragment_address, fragment_attribute_mask, mapped_fragment_attribute_mask },
            );
        ...
        if (vertex_stage == .export_shader and vertex_scalar_count < vertex_scalar_regs.len) {
            // GFX10 merged shaders reserve s0:s7 for SPI inputs. Unity's NGG
            // prolog reads the active ES/GS lane counts from the low two bytes
            // of s3 before it touches USER_DATA at s8. Vulkan invokes exactly
            // the live vertices, so a full 64/64 wave is the stable equivalent;
            // copying USER_DATA[3] here corrupts EXEC and collapses geometry.
            vertex_scalar_regs[vertex_scalar_count] = .{
                .register = 3,
                .value = 0x0000_4040,
            };
        ...
        {
            for (graphics_resources.descriptors[0..fragment_image_count]) |source| {
        ...
        if (self.shouldReportVertexResources(vertex_address)) {
            ...
            for (vertex_storage.mappings[0..vertex_storage.mapping_count]) |mapping| {
                const slot: usize = @intCast(mapping.descriptor_index);
                std.debug.print(
                    "  pc={any} V#s{d} slot={d} addr=0x{x} size=0x{x} stride={d} fmt={d} dst={any} soffset={any} vertex_attribute={any}\n",
                    .{
                        mapping.instruction_pc,
                        mapping.resource_sgpr,
                        mapping.descriptor_index,
        ...
        if (self.traceCurrentGraphicsFrame()) {
            std.debug.print(
                "[vulkan dcb] traced vertex resources draw={d} stage={s} program=0x{x} mappings={d} index=0x{x}/{?d} index32={any} base={d} first={d} instances={d}\n",
                .{
                    self.frame_profile.draws,
            ...
            for (vertex_storage.mappings[0..vertex_storage.mapping_count]) |mapping| {
                const slot: usize = @intCast(mapping.descriptor_index);
                std.debug.print(
                    "  trace V#s{d} slot={d} addr=0x{x} size=0x{x} stride={d} fmt={d} dst={any} soffset={any} attr={any}\n",
                    .{
                        mapping.resource_sgpr,
                        mapping.descriptor_index,
                        vertex_storage.addresses[slot],
            ...
            if (draw.index_address != 0 and draw.index_count != null and draw.index_count.? != 0) {
                ...
                if (reader.read(draw.index_address, &index_bytes)) |_| {
                    std.debug.print("  trace indices", .{});
                    if (draw.index_uint32) {
                        var offset: usize = 0;
                        while (offset < index_bytes.len) : (offset += 4) {
                            std.debug.print(" {d}", .{std.mem.readInt(u32, index_bytes[offset..][0..4], .little)});
                        }
                    } else {
                        var offset: usize = 0;
                        while (offset < index_bytes.len) : (offset += 2) {
                            std.debug.print(" {d}", .{std.mem.readInt(u16, index_bytes[offset..][0..2], .little)});
                        }
                    }
                    std.debug.print("\n", .{});
                } else |_| {}
        ...
        if (self.force_probe_fragment_ui and fragment_mapping_count != 0) {
        ...
        var unity_ui_position: ?gpu.ShaderSpirvStorageBufferBinding = null;
        ...
        for (vertex_storage.mappings[0..vertex_storage.mapping_count]) |mapping| {
            ...
            // fetch order silently swaps the values: 0..1 is then projected
            // as a screen position and every primitive collapses at a corner.
            if (mapping.soffset_value == 16 and mapping.unified_format == 64) {
                unity_ui_uv = mapping;
            } else if (mapping.soffset_value == 24 and mapping.unified_format == 64) {
                unity_ui_position = mapping;
            } else if (mapping.soffset_value == 32 and mapping.unified_format == 56) {
        ...
        if (unity_ui_fallback) {
            unity_ui_fragment_module = try buildUnityUiFragmentSpirv(
                ...
                graphics_resources.mappings[0],
                .{ target.descriptor.width, target.descriptor.height },
                if (graphics_resources.descriptors[0].unified_format == 1) blk: {
                    for (graphics_resources.descriptors[0].dst_select, 0..) |selector, component| {
                        if (selector == 4) break :blk @as(u32, @intCast(component));
                    }
                    break :blk @as(u32, 0);
                } else null,
            );
        ...
        else if (unity_ui_fragment_module) |module|
            module.words
        ...
        if (try_guest_vs) {
            ...
            if (vertex_module_result) |vertex_module_owned| {
                ...
                if (unity_ui_fallback and !self.reported_ui_composite) {
                    self.reported_ui_composite = true;
                    std.debug.print(
                        "[vulkan dcb] using direct Unity UI vertex/texture fallback @0x{x} {d}x{d}\n",
                        .{ target.descriptor.address, target.descriptor.width, target.descriptor.height },
                    );
                }
                if (log_verbose_gpu) std.debug.print(
                ...
                const rectangle_completion = rectangleCompletionMode(
                    vertex_stage,
                    render_state.primitive_type,
                    draw,
                    fullscreen_corner,
                );
                if (self.geometry_shaders_available and rectangle_completion != 0) {
                ...
                if (unity_ui_fallback) {
                    ...
                    // faithful yet. Once the UI layer itself is translated,
    ...
    fn emulateFullscreenSampleBlit(
        ...
        const source_format = sampledImageFormat(source.unified_format, false) orelse return false;
        if (self.findResidentRenderTargetIndex(source, source_format)) |source_index| {
        ...
        // The CPU fallback below currently detiles ordinary RGBA8. Packed HDR
        // sources are handled by the resident Vulkan blit above, which also
        // performs the format conversion into the scanout-compatible target.
        if (source.unified_format != 56) return false;
    ...
    fn prepareGraphicsResources(
        ...
        errdefer result.deinit(self);
        try self.appendGraphicsResources(&result, bindings, reader, analysis);
        return result;
    }

    fn appendGraphicsResources(
        self: *Renderer,
        ...
        for (analysis.program.instructions.items) |inst| {
            ...
            const image_slot = graphicsSrtImageSlot(
                result.mappings[stage_mapping_start..result.mapping_count],
                inst.src1.reg,
            );
    ...
    fn commitStorageImages(self: *Renderer, memory: GuestMemory, resources: *const ComputeResources) anyerror!void {
        ...
        for (resources.storage_images[0..resources.storage_image_count]) |prepared| {
            ...
            const cache_index = prepared.cache_index orelse return Error.UnsupportedStorageImage;
            if (cache_index >= self.storage_image_cache.items.len) return Error.UnsupportedStorageImage;
            const cached = &self.storage_image_cache.items[cache_index];
    ...
    fn stageResidentStorageImage(
        ...
        for (self.storage_image_cache.items, 0..) |*cached, index| {
            ...
            const view = try self.residentImageView(
                cached.image.handle,
                if (dimension == .three_d) vk.image_view_type_3d else vk.image_view_type_2d,
                image_format,
                components,
                vk.image_aspect_color_bit,
            );
    ...
    fn stageSampledImage(
        ...
        // alias generation above still rejects any deferred GPU writer. This
        ...
        for (self.sampled_image_cache.items) |*item| {
            if (item.guest_address != descriptor.address or
                ...
                item.image_type != @intFromEnum(descriptor.image_type) or
                item.dimension != dimension or
                item.tile_mode != @intFromEnum(descriptor.tile_mode) or
        ...
        self.frame_profile.sampled_flush_ns +|= elapsedHostNanoseconds(flush_started);
        // A flush may have published GPU bytes into guest pages and advanced
        // their generation. Drop the pre-flush probe before building the cache
        ...
        self.frame_profile.sampled_lookup_ns +|= elapsedHostNanoseconds(lookup_started);
        ...
        if (false and self.traceCurrentGraphicsFrame() and bytes_per_texel == 4 and
    ...
    fn combineSourceGenerations(resident: u64, pages: u64) u64 {
        if (pages == 0) return resident;
        var combined = pages ^ std.math.rotl(u64, resident, 23) ^ 0x9e37_79b9_7f4a_7c15;
        combined *%= 0xbf58_476d_1ce4_e5b9;
        combined ^= combined >> 31;
        return if (combined == 0) 1 else combined;
    }
    ...
    fn residentSampler(self: *Renderer, descriptor: gpu.resources.SamplerDescriptor) anyerror!vk.Sampler {
        for (self.resident_samplers.items) |cached| {
            if (std.meta.eql(cached.descriptor, descriptor)) return cached.sampler;
        }
        const sampler = try self.createGuestSampler(descriptor);
        errdefer self.device_functions.destroy_sampler(self.device, sampler, null);
        try self.resident_samplers.append(self.allocator, .{
            .descriptor = descriptor,
            .sampler = sampler,
        });
        return sampler;
    ...
    fn residentImageView(
        ...
        const info = vk.ImageViewCreateInfo{
            ...
            .format = format,
            .components = components,
            .subresource_range = .{ .aspect_mask = aspect_mask },
        };
        var view: vk.ImageView = 0;
        ...
        try self.resident_image_views.append(self.allocator, .{
            .image = image,
            .view_type = view_type,
            .format = format,
            .components = components,
    ...
    fn invalidateResidentImageViews(self: *Renderer, image: vk.Image) void {
        var index: usize = 0;
        while (index < self.resident_image_views.items.len) {
            const cached = self.resident_image_views.items[index];
            if (cached.image != image) {
                index += 1;
                continue;
            }
            self.destroyImageView(cached.view);
            _ = self.resident_image_views.swapRemove(index);
        }
    }
    ...
    fn waitForTick(self: *Renderer, tick: u64) Error!void {
        if (tick == 0) return;
        try self.refreshGpuProgress();
        if (tick <= self.completed_tick) return;
        const semaphores = [_]vk.Semaphore{self.timeline_semaphore};
        const values = [_]u64{tick};
    ...
    fn waitForSubmittedWork(self: *Renderer) Error!void {
    ...
    fn ensureFrameCommandBuffer(self: *Renderer) (Error || std.mem.Allocator.Error)!usize {
        try self.refreshGpuProgress();
        if (self.reusableCommandBuffer()) |index| return index;
        if (self.frame_command_buffers.items.len >= maximum_async_command_buffers) {
        ...
        var buffers: [32]vk.CommandBuffer = undefined;
        const allocate_info = vk.CommandBufferAllocateInfo{
            .command_pool = self.command_pool,
            .level = vk.command_buffer_level_primary,
            .command_buffer_count = buffers.len,
        };
        if (self.device_functions.allocate_command_buffers(
            self.device,
            &allocate_info,
            &buffers,
        ) != vk.success) return Error.CommandBufferAllocationFailed;
        const first = self.frame_command_buffers.items.len;
        self.frame_command_buffers.appendSlice(self.allocator, &buffers) catch |err| {
            self.device_functions.free_command_buffers(
                self.device,
                self.command_pool,
                buffers.len,
                &buffers,
        ...
        self.command_buffer_ticks.appendNTimes(self.allocator, 0, buffers.len) catch |err| {
            self.frame_command_buffers.shrinkRetainingCapacity(first);
            self.device_functions.free_command_buffers(
                self.device,
                self.command_pool,
                buffers.len,
    ...
    fn beginOneShot(self: *Renderer) (Error || std.mem.Allocator.Error)!vk.CommandBuffer {
        ...
        const command_buffer = self.frame_command_buffers.items[slot];
    ...
    fn releaseOneShot(self: *Renderer, command_buffer: vk.CommandBuffer) void {
            ...
            self.recording_command_buffer.? == command_buffer)
        {
            ...
            if (self.recording_command_slot) |slot| self.command_buffer_ticks.items[slot] = 0;
            self.recording_command_buffer = null;
            self.recording_command_slot = null;
        }
        for (self.frame_command_buffers.items) |owned| {
            if (owned == command_buffer) return;
        }
    ...
    fn submitOneShot(self: *Renderer, command_buffer: vk.CommandBuffer) (Error || std.mem.Allocator.Error)!void {
        ...
        try self.pending_command_buffers.append(self.allocator, command_buffer);
        ...
        if (self.draw_batch_active) {
            if (self.current_descriptor_slot) |descriptor_slot| {
                var found = false;
                for (self.pending_descriptor_slots.items) |pending| {
                    if (pending == descriptor_slot) {
                        found = true;
                        break;
                    }
                }
    ...
    fn flushQueuedCommands(self: *Renderer) Error!void {
        ...
        for (self.pending_descriptor_slots.items) |slot| {
            self.descriptor_slot_ticks[slot] = signal_tick;
        }
        self.pending_command_buffers.clearRetainingCapacity();
        self.pending_command_slots.clearRetainingCapacity();
    ...
    fn beginDescriptorBatch(
        ...
        // submitted slots are present in pending_descriptor_slots and keep
        // their timeline ownership.
        if (self.current_descriptor_slot) |current| {
            var pending = false;
            for (self.pending_descriptor_slots.items) |slot| {
                if (slot == current) {
                    pending = true;
            ...
            if (!pending and self.descriptor_slot_ticks[current] == command_buffer_pending_tick) {
                self.descriptor_slot_ticks[current] = 0;
            }
        }
        var selected: ?usize = null;
        var searched: usize = 0;
        while (searched < self.descriptor_sets.len) : (searched += 1) {
            const slot = (self.descriptor_cursor + searched) % self.descriptor_sets.len;
            const tick = self.descriptor_slot_ticks[slot];
            if (tick != command_buffer_pending_tick and tick <= self.completed_tick) {
                selected = slot;
                break;
            }
        }
        if (selected == null) {
            try self.flushQueuedCommands();
            var oldest_tick: u64 = std.math.maxInt(u64);
            for (self.descriptor_slot_ticks) |tick| {
                if (tick != command_buffer_pending_tick) oldest_tick = @min(oldest_tick, tick);
            }
            if (oldest_tick != std.math.maxInt(u64)) try self.waitForTick(oldest_tick);
            searched = 0;
            while (searched < self.descriptor_sets.len) : (searched += 1) {
                const slot = (self.descriptor_cursor + searched) % self.descriptor_sets.len;
                if (self.descriptor_slot_ticks[slot] <= self.completed_tick) {
                    selected = slot;
                    break;
                }
            }
        }
        const slot = selected orelse return Error.DescriptorSetAllocationFailed;
        self.draw_batch_active = true;
        self.draw_uploads_enabled = allow_draw_uploads;
        self.current_descriptor_slot = slot;
        ...
        @memset(self.dynamic_scalar_mapping.?[0..dynamic_scalar_buffer_words], 0);
    }

    fn beginFrameDraw(self: *Renderer) (Error || std.mem.Allocator.Error)!void {
        return self.beginDescriptorBatch(true);
    }

    fn beginComputeDispatch(self: *Renderer) (Error || std.mem.Allocator.Error)!void {
        return self.beginDescriptorBatch(false);
    }
    ...
    fn finishDrawBatch(self: *Renderer) Error!void {
        ...
        if (self.current_descriptor_slot) |slot| {
            if (self.descriptor_slot_ticks[slot] == command_buffer_pending_tick) {
                self.descriptor_slot_ticks[slot] = 0;
            }
        }
        self.current_descriptor_slot = null;
        self.resetDrawRings();
    }

    fn allocateDrawUpload(self: *Renderer, bytes: usize) Error!DrawUploadSlice {
        var offset = reserveAlignedRange(
            &self.draw_upload_offset,
            draw_upload_ring_bytes,
            bytes,
            draw_upload_alignment,
        );
        if (offset == null) {
    ...
    fn deferVulkanObject(self: *Renderer, object: DeferredVulkanObject) void {
        const retire_tick = if (self.pending_command_buffers.items.len != 0 or
            self.recording_command_buffer != null)
            self.submitted_tick +% 1
        else
            self.submitted_tick;
        if (retire_tick == 0 or retire_tick <= self.completed_tick) {
            self.destroyVulkanObject(object);
            return;
        }
        // Leaking one transient object on host OOM is safer than destroying an
        // object still referenced by a recorded Vulkan command.
        self.deferred_vulkan_objects.append(self.allocator, .{
    ...
    fn destroyVulkanObject(self: *Renderer, object: DeferredVulkanObject) void {
        switch (object) {
            ...
            .image_view => |view| self.device_functions.destroy_image_view(self.device, view, null),
            .sampler => |sampler| self.device_functions.destroy_sampler(self.device, sampler, null),
            .render_pass => |render_pass| self.device_functions.destroy_render_pass(self.device, render_pass, null),
            .framebuffer => |framebuffer| self.device_functions.destroy_framebuffer(self.device, framebuffer, null),
            .pipeline => |pipeline| self.device_functions.destroy_pipeline(self.device, pipeline, null),
    ...
    fn destroyDeferredVulkanObjects(self: *Renderer) void {
        var index: usize = 0;
        while (index < self.deferred_vulkan_objects.items.len) {
            const entry = self.deferred_vulkan_objects.items[index];
            if (entry.retire_tick > self.completed_tick) {
                index += 1;
                continue;
            }
            self.destroyVulkanObject(entry.object);
            _ = self.deferred_vulkan_objects.swapRemove(index);
        }
    }
    ...
    fn destroyImage(self: *Renderer, image: OwnedImage) void {
        self.deferVulkanObject(.{ .image = image });
    }

    fn destroyImageView(self: *Renderer, view: vk.ImageView) void {
        self.deferVulkanObject(.{ .image_view = view });
    }

    fn destroySampler(self: *Renderer, sampler: vk.Sampler) void {
        self.deferVulkanObject(.{ .sampler = sampler });
    }

    fn destroyRenderPass(self: *Renderer, render_pass: vk.RenderPass) void {
        self.deferVulkanObject(.{ .render_pass = render_pass });
    }

    fn destroyFramebuffer(self: *Renderer, framebuffer: vk.Framebuffer) void {
        self.deferVulkanObject(.{ .framebuffer = framebuffer });
    }

    fn destroyPipeline(self: *Renderer, pipeline: vk.Pipeline) void {
        self.deferVulkanObject(.{ .pipeline = pipeline });
    }

    fn destroyShaderModule(self: *Renderer, shader: vk.ShaderModule) void {
        self.deferVulkanObject(.{ .shader_module = shader });
    }
    ...
    fn readMapped(self: *Renderer, buffer: OwnedBuffer, destination: []u8) Error!void {
        return self.readMappedAt(buffer, 0, destination);
    }
    ...
    fn synchronizeDrawBatch(self: *Renderer, operation: []const u8) bool {
        ...
        self.waitForSubmittedWork() catch |err| {
            self.last_sync_error = err;
            std.debug.print(
                "[vulkan dcb] {s} timeline wait failed: {s}\n",
                .{ operation, @errorName(err) },
            );
            return false;
        };
    ...
    fn dcbRead(context: ?*anyopaque, address: u64, bytes: []u8) bool {
        ...
        if (!self.synchronizeDrawBatch("read")) return false;
    ...
    fn dcbWrite(context: ?*anyopaque, address: u64, bytes: []const u8) bool {
        const self = fromContext(context);
        if (!self.synchronizeDrawBatch("write")) return false;
    ...
    fn dcbRelease(context: ?*anyopaque, release: gpu.state.ReleaseMem) bool {
        ...
        self.finishDrawBatch() catch |err| {
            self.last_sync_error = err;
            std.debug.print("[vulkan dcb] release batch failed: {s}\n", .{@errorName(err)});
            return false;
        };
        self.waitForSubmittedWork() catch |err| {
            self.last_sync_error = err;
        ...
        if (!accepted) {
            if (release.data_selection >= 1 and release.data_selection <= 4) {
                self.last_sync_error = Error.GuestMemoryWriteFailed;
                std.debug.print(
                    "[vulkan dcb] release label not CPU-visible: {s} (address=0x{x}, selection={d})\n",
                    .{ @errorName(Error.GuestMemoryWriteFailed), release.address, release.data_selection },
                );
            }
            // A release target may be a GPU-only VA with no CPU mapping. The
            // label cannot be mirrored into guest memory in that case, but the
            // command buffer still completed and its AGC interrupt must be
    ...
    fn dcbWriteData(context: ?*anyopaque, info: gpu.state.WriteData, values: []const u32) bool {
        ...
        if (!self.synchronizeDrawBatch("write-data")) return false;
    ...
    fn dcbDmaData(context: ?*anyopaque, dma: gpu.state.DmaData) bool {
        ...
        switch (dma.destination) {
            0, 3 => {
                // Engines emit four-byte immediate-to-L2 ordering markers with
                // null or tiny sentinel destinations between workloads. Those
                // addresses cannot name guest memory (the first guest page is
    ...
    /// choosing would be.
    fn latestAliasedDisplayTarget(self: *Renderer, buffer: DisplayBuffer) ?usize {
        var selected: ?usize = null;
        var selected_sequence: u64 = 0;
        for (self.render_targets.items, 0..) |target, index| {
            if (!target.initialized or
                target.target.descriptor.address == buffer.address or
                target.target.descriptor.width != buffer.width or
                target.target.descriptor.height != buffer.height or
                target.target.format.bytes_per_texel != 4)
            {
                continue;
            }
            if (selected == null or target.last_used_sequence > selected_sequence) {
                selected = index;
                selected_sequence = target.last_used_sequence;
            }
        }
        return selected;
    }
    ...
    fn latestVisibleCompletedFrame(self: *Renderer, buffer: DisplayBuffer) ?usize {
        ...
        var matching_sequence: u64 = 0;
        var fallback: ?usize = null;
        var fallback_area: u64 = 0;
        var fallback_sequence: u64 = 0;
        for (self.completed_frames.items, 0..) |frame, index| {
                ...
                (area == fallback_area and frame.sequence > fallback_sequence))
            {
                fallback = index;
                fallback_area = area;
                fallback_sequence = frame.sequence;
            }
            ...
            {
                matching_rgba = index;
                matching_rgba_sequence = frame.sequence;
            }
            if (matching == null or frame.sequence > matching_sequence) {
                matching = index;
                matching_sequence = frame.sequence;
            }
        }
    ...
    fn presentGuestBuffer(self: *Renderer, buffer: DisplayBuffer, flip: gpu.state.Flip) bool {
        ...
        // Alpha alone does not make a scanout visibly populated. VideoOut
        // buffers are commonly cleared to opaque black before the GPU alias is
        // rendered, so count RGB here instead of treating A=255 as content.
        const guest_nz = countNonblackRgb(self.guest_frame_scratch.items);
        if (guest_nz == 0) {
            // Some titles register a CPU-visible VideoOut alias while their CB
            // descriptors name a separate GPU mapping of the same framebuffer.
            // An exact-address lookup cannot connect those VAs. The most recent
            // full-size attachment is the GPU side of that alias; blitting it
            // directly also preserves packed HDR-to-swapchain conversion.
                    ...
                    if (self.flip_callbacks <= 8 or log_verbose_gpu) {
                        std.debug.print(
                            ...
                            .{
                                ...
                                buffer.address,
                                target.last_used_sequence,
                                target.target.descriptor.format,
                            },
                        );
                    }
                    return true;
            ...
            if (self.latestVisibleCompletedFrame(buffer)) |idx| {
                ...
                if (self.flip_callbacks <= 8 or log_verbose_gpu) {
                    std.debug.print(
                        ...
                        .{ cached.guest_address, buffer.address, cached.sequence, cached.width, cached.height },
                    );
                }
    ...
    /// Captures only the most recently used full-size resident attachments at
    /// one opt-in progress checkpoint. This is substantially cheaper than the
    /// per-draw graphics trace on a frame containing hundreds of draws and
    /// reveals the real compositor output when VideoOut still names a black
    /// alias.
    fn dumpRecentResidentTargets(self: *Renderer) void {
        const maximum_candidates = 16;
        var candidates: [maximum_candidates]usize = undefined;
        var sequences: [maximum_candidates]u64 = undefined;
        var candidate_count: usize = 0;

        for (self.render_targets.items, 0..) |cached, target_index| {
            const descriptor = cached.target.descriptor;
            if (!cached.initialized or descriptor.fragments_log2 != 0 or
                descriptor.width < 640 or descriptor.height < 360 or
                (cached.target.format.bytes_per_texel != 4 and
                    cached.target.format.bytes_per_texel != 8))
            {
                continue;
            }

            var insert_at: usize = 0;
            while (insert_at < candidate_count and
                sequences[insert_at] >= cached.last_used_sequence) : (insert_at += 1)
            {}
            if (insert_at >= maximum_candidates) continue;
            const move_end = @min(candidate_count, maximum_candidates - 1);
            var move_at = move_end;
            while (move_at > insert_at) : (move_at -= 1) {
                candidates[move_at] = candidates[move_at - 1];
                sequences[move_at] = sequences[move_at - 1];
            }
            candidates[insert_at] = target_index;
            sequences[insert_at] = cached.last_used_sequence;
            candidate_count = @min(candidate_count + 1, maximum_candidates);
        }

        for (candidates[0..candidate_count], 0..) |target_index, rank| {
            self.materializeRenderTarget(target_index) catch |err| {
                std.debug.print(
                    "[vulkan resident] rank={d} materialize failed: {s}\n",
                    .{ rank, @errorName(err) },
                );
                continue;
            };
            const target = self.render_targets.items[target_index].target;
            var selected: ?*const CachedFrame = null;
            var selected_sequence: u64 = 0;
            for (self.completed_frames.items) |*frame| {
                const frame_target = frame.target orelse continue;
                if (!sameRenderTarget(frame_target, target) or
            ...
            const frame = selected orelse continue;
            var path_buffer: [192]u8 = undefined;
            const path = std.fmt.bufPrintZ(
                &path_buffer,
                "out\\resident-frame-{d:0>4}-rank-{d:0>2}-0x{x}-{d}x{d}-fmt-{d}.ppm",
                .{
                    self.flip_callbacks,
                    rank,
                    target.descriptor.address,
            ...
            std.debug.print(
                "[vulkan resident] rank={d} sequence={d} @0x{x} {d}x{d} fmt={d} bytes={d}\n",
                .{
                    rank,
                    selected_sequence,
                    target.descriptor.address,
        ...
        if (should_print) {
            ...
            std.debug.print(
                ...
                .{
                    ...
                    profile.compute_pipeline_hits,
                    profile.compute_pipeline_misses,
                    profile.compute_pipeline_build_ns / std.time.ns_per_ms,
                    profile.compute_emulation_ns / std.time.ns_per_ms,
                    profile.compute_resource_ns / std.time.ns_per_ms,
                    ...
                    profile.sampled_stage_ns / std.time.ns_per_ms,
                    profile.sampled_flush_ns / std.time.ns_per_ms,
                    profile.sampled_generation_ns / std.time.ns_per_ms,
                    profile.sampled_lookup_ns / std.time.ns_per_ms,
    ...
    fn tryPresentFlip(self: *Renderer, flip: gpu.state.Flip) bool {
        ...
        if (self.display_buffer_resolver) |resolver| {
            ...
            if (self.flip_callbacks <= 8) {
                std.debug.print(
                    "[vulkan dcb] flip #{d} requests @0x{x} {d}x{d} pitch={d} tile={d} completed={d} resident={d}\n",
                    .{
                        self.flip_callbacks,
                        requested.?.address,
                        requested.?.width,
                        requested.?.height,
                        requested.?.pitch_in_pixels,
                        requested.?.tiling_mode,
                        self.completed_frames.items.len,
                        self.render_targets.items.len,
                    },
                );
            }
            ...
            // The deferred-composite input can be produced after the pass that
            // first identifies it, so the draw-time fast path above may not see
            // it yet.  Resolve it after the whole DCB batch, when the intact
            // resident RGBA scene is guaranteed to be available.
            if (self.window_presentation != null) {
                // The guest renders this UI target only when its contents
                // change, then keeps flipping an unresolved composite buffer.
                ...
                if (self.deferred_composite_ui_address != null) {
                    ...
                    for (self.render_targets.items, 0..) |candidate, candidate_index| {
                        ...
                        if (ui_index == null or candidate.last_used_sequence > ui_sequence) {
                            ui_index = candidate_index;
                            ui_sequence = candidate.last_used_sequence;
                        }
                    }
                    if (ui_index) |target_index| {
                        ...
                        if (self.flip_callbacks <= 24 or self.flip_callbacks == 64 or log_verbose_gpu) {
                            std.debug.print(
                                "[vulkan dcb] presenting preserved deferred-composite layer @0x{x}\n",
                                .{ui_address},
                            );
                        }
                        self.presented_frames += 1;
        ...
        var frame_index = selected_index orelse {
        ...
        if (requested) |buffer| {
            const selected = &self.completed_frames.items[frame_index];
            if (countNonblackRgb(selected.pixels.items) == 0) {
                var replaced_with_alias = false;
                if (self.latestAliasedDisplayTarget(buffer)) |target_index| {
                    self.materializeRenderTarget(target_index) catch |err| {
                        self.last_flip_error = err;
                        return false;
                    };
                    const alias_address = self.render_targets.items[target_index].target.descriptor.address;
                    var alias_frame: ?usize = null;
                    var alias_sequence: u64 = 0;
                    for (self.completed_frames.items, 0..) |candidate, candidate_index| {
                        if (candidate.guest_address != alias_address or
                            countNonblackRgb(candidate.pixels.items) == 0 or
                            (alias_frame != null and candidate.sequence <= alias_sequence))
                        {
                            continue;
                        }
                        alias_frame = candidate_index;
                        alias_sequence = candidate.sequence;
                    }
                    if (alias_frame) |alias_index| {
                        frame_index = alias_index;
                        replaced_with_alias = true;
                        if (self.flip_callbacks <= 8 or log_verbose_gpu) {
                            const alias = &self.completed_frames.items[alias_index];
                            std.debug.print(
                                "[vulkan dcb] replacing black exact target @0x{x} with full-size alias @0x{x} sequence={d} fmt={d}\n",
                                .{ buffer.address, alias.guest_address, alias.sequence, alias.target.?.descriptor.format },
                            );
                        }
                    }
                }
                if (!replaced_with_alias) {
                    if (self.latestVisibleCompletedFrame(buffer)) |visible_index| {
                        frame_index = visible_index;
                        const visible = &self.completed_frames.items[visible_index];
                        if (self.flip_callbacks <= 8 or log_verbose_gpu) {
                            std.debug.print(
                                "[vulkan dcb] replacing black exact target @0x{x} with visible target @0x{x} sequence={d} {d}x{d}\n",
                                .{ buffer.address, visible.guest_address, visible.sequence, visible.width, visible.height },
                            );
                        }
                    }
                }
    ...
    fn decodeDirectGuestDraw(packet: gpu.pm4.Packet, state: *const gpu.State) ?GuestDraw {
        ...
        const index_offset_bits = state.readRegister(.uconfig, 0x24a) orelse 0; // GE_INDX_OFFSET
        if (packet.opcode == gpu.pm4.draw_index_auto and packet.body.len >= 1)
            return .{
                ...
                .first_vertex = index_offset_bits,
            };
        if (packet.opcode == gpu.pm4.draw_index_2 and packet.body.len >= 5)
            return .{
                .instance_count = state.instance_count,
                .vertex_offset = @bitCast(index_offset_bits),
                .index_count = packet.body[3],
                .index_address = (@as(u64, packet.body[2]) << 32) | packet.body[1],
                // INDEX_TYPE is persistent command-processor state and applies
                // to both address forms. Treating every embedded-address draw
                // as u16 splits a u32 stream such as 0,1,2 into
                // 0,0,1,0,2,0 and collapses half of the guest geometry.
    ...
    fn dcbDraw(context: ?*anyopaque, state: *const gpu.State, packet: gpu.pm4.Packet) bool {
        ...
        defer if (trace_stall) std.debug.print(
            "[vulkan stall] end draw={d} elapsed_ms={d}\n",
            .{ self.frame_profile.draws, elapsedHostNanoseconds(profile_started) / std.time.ns_per_ms },
        );
            ...
            while (index < draw_count) : (index += 1) {
                ...
                if (self.capture_extended_progress_frames and self.flip_callbacks == 63) {
                    ...
                    for (render_state.color_targets) |candidate| {
                        ...
                        target_address = color.address;
                        target_width = color.width;
                        target_height = color.height;
                        break;
                    }
                    std.debug.print(
                        "[vulkan geometry] draw={d} primitive={d} vertices={d}->{d} instances={d} indices={?d} index32={any} base_vertex={d} index_va=0x{x} target=0x{x}/{d}x{d}\n",
                        .{
                            self.frame_profile.draws,
                            render_state.primitive_type,
                            decoded_draw.vertex_count,
                            draw.vertex_count,
                            draw.instance_count,
                            draw.index_count,
                            draw.index_uint32,
                ...
                self.drawGuestGraphics(state, draw, vertex_stage.?, null) catch |err| {
                    ...
                    if (self.traceCurrentGraphicsFrame()) {
                        std.debug.print(
                            "[vulkan dcb] traced draw failed draw={d}: {s}\n",
                            .{ self.frame_profile.draws, @errorName(err) },
                        );
                    }
    ...
    fn dcbDispatch(context: ?*anyopaque, state: *const gpu.State, packet: gpu.pm4.Packet) bool {
        ...
        if (trace_stall) std.debug.print(
            ...
            .{
                self.frame_profile.dispatches,
                packet.opcode,
                gpu.resources.ShaderStage.compute.programAddress(state),
            },
        );
        defer if (trace_stall) std.debug.print(
        ...
        _ = self.dispatchRdna2State(
            ...
            const soft = err == Error.MissingStorageDescriptor or
                ...
                std.mem.eql(u8, @errorName(err), "IncompleteDescriptor") or
                std.mem.eql(u8, @errorName(err), "InvalidDescriptor") or
                std.mem.eql(u8, @errorName(err), "InvalidFormat") or
...
///
/// RELEASE_MEM closes and fence-waits the preceding frame batch before this
/// invariant host counter is sampled, preserving the ordering and progress
/// properties titles rely on. A Vulkan timestamp query can replace this value
/// when release packets are recorded on the device queue itself.
...
fn sameShaderOperand(a: gpu.ShaderOperand, b: gpu.ShaderOperand) bool {
    return a.kind == b.kind and a.reg == b.reg and a.value == b.value;
}

fn shaderOperandAt(source: gpu.ShaderOperand, component: u8) ?gpu.ShaderOperand {
    if (component == 0) return source;
    return switch (source.kind) {
        .vgpr, .sgpr => blk: {
            var result = source;
            result.reg = std.math.add(u32, source.reg, component) catch return null;
            break :blk result;
        },
        else => null,
    };
...
fn shaderOperandConstantBefore(
    ...
    switch (source.kind) {
        .integer_inline_constant, .literal_constant, .float_inline_constant, .null => return source.value,
        .vgpr => {},
        else => return null,
    }
...
fn inferNggLdsExports(
    ...
    const terminal = terminal_index orelse return 0;

    var words: [64]NggLdsWord = @splat(.{});
    var base: ?gpu.ShaderOperand = null;
    var index = terminal;
    while (index != 0) {
        index -= 1;
        const inst = instructions[index];
        if (inst.opcode.isBranch()) break;
        const is_write = switch (inst.opcode) {
            .ds_write_b32, .ds_write2_b32, .ds_write_b64, .ds_write_b96, .ds_write_b128 => true,
            else => false,
        };
        if (!is_write or inst.gds) continue;
        if (base == null) base = inst.src0;
        ...
        switch (inst.opcode) {
            ...
            .ds_write_b32, .ds_write_b64, .ds_write_b96, .ds_write_b128 => {
                ...
                var component: u8 = 0;
                while (component < count) : (component += 1) {
                    const source = shaderOperandAt(inst.src1, component) orelse return 0;
                    recordNggLdsWord(
                        &words,
    ...
    if (!words[0].present or !words[1].present) return 0;

    var position_zw: ?usize = null;
    ...
    // Scene vertices have dynamic clip-space Z/W, so the constant 0/1 marker
    // used by fullscreen passes is not available. The ABI is otherwise fully
    // determined by the pixel shader's live PARAM components: POS.xy, those
    // packed PARAM words, then POS.zw. Use that boundary when both words were
    // recovered from the terminal LDS record.
    var expected_zw: usize = 2;
    for (parameter_components) |component_mask| {
        expected_zw += @popCount(component_mask);
    }
    const zw = position_zw orelse blk: {
        if (expected_zw + 1 >= words.len or
            !words[expected_zw].present or !words[expected_zw + 1].present)
        {
            return 0;
        }
...
fn countNonblackRgb(linear: []const u8) u32 {
    ...
    var i: usize = 0;
    while (i + 3 < linear.len) : (i += 4) {
        if (linear[i] != 0 or linear[i + 1] != 0 or linear[i + 2] != 0) nonblack += 1;
    }
    return nonblack;
}

fn decodeUnsignedMiniFloat(raw: u32, width: u5) f32 {
    const mantissa_bits: u5 = width - 5;
    const mantissa_mask = (@as(u32, 1) << mantissa_bits) - 1;
    const mantissa = raw & mantissa_mask;
    const exponent = (raw >> mantissa_bits) & 0x1f;
    if (exponent == 0) {
        const scale: f32 = if (mantissa_bits == 6) 1.0 / 1_048_576.0 else 1.0 / 524_288.0;
        return @as(f32, @floatFromInt(mantissa)) * scale;
    }
    if (exponent == 0x1f) return std.math.inf(f32);
    const bits = ((exponent + 112) << 23) | (mantissa << (23 - mantissa_bits));
    return @bitCast(bits);
}

fn linearFloatToUnorm8(value: f32) u8 {
    if (!std.math.isFinite(value)) return 255;
    const clamped = std.math.clamp(value, 0.0, 1.0);
    return @intFromFloat(clamped * 255.0 + 0.5);
}

fn convertR11G11B10ToRgba8(source: []const u8, destination: []u8) void {
    ...
    while (offset < source.len) : (offset += 4) {
        ...
        destination[offset] = linearFloatToUnorm8(decodeUnsignedMiniFloat(word & 0x7ff, 11));
...
fn isBlockCompressedUnifiedFormat(unified_format: u16) bool {
...
fn dumpFrameThumbnailPpm(path: [*:0]const u8, width: u32, height: u32, rgba: []const u8) void {
    const divisor: u32 = 8;
    const output_width = @max(@as(u32, 1), width / divisor);
    const output_height = @max(@as(u32, 1), height / divisor);
    if (rgba.len < @as(usize, width) * @as(usize, height) * 4) return;
    ...
    for (0..output_height) |y| {
        for (0..output_width) |x| {
            ...
            @memcpy(output[destination..][0..4], rgba[source..][0..4]);
        }
    }
    dumpFramePpm(path, output_width, output_height, output);
}

fn dumpR11G11B10FrameThumbnailPpm(
    path: [*:0]const u8,
    width: u32,
    height: u32,
...
fn dumpRgba16FloatFrameThumbnailPpm(
    path: [*:0]const u8,
    width: u32,
    height: u32,
    rgba16: []const u8,
    ...
    const output_width = @max(@as(u32, 1), width / divisor);
    const output_height = @max(@as(u32, 1), height / divisor);
    if (rgba16.len < @as(usize, width) * @as(usize, height) * 8) return;
    const output = std.heap.page_allocator.alloc(
        u8,
        @as(usize, output_width) * @as(usize, output_height) * 4,
    ) catch return;
    ...
    for (0..output_height) |y| {
        for (0..output_width) |x| {
            const source_x = @min(width - 1, @as(u32, @intCast(x)) * divisor);
            const source_y = @min(height - 1, @as(u32, @intCast(y)) * divisor);
            const source = (@as(usize, source_y) * width + source_x) * 8;
            const destination = (y * output_width + x) * 4;
            for (0..3) |component| {
                const bits = std.mem.readInt(u16, rgba16[source + component * 2 ..][0..2], .little);
...
/// supplies one invocation per final indexed vertex, so replaying that wave
...
fn buildUnityUiVertexSpirv(
    ...
    try program.instructions.appendSlice(allocator, &.{
        ...
        // Present a simple, direct interface to the matching fallback PS.
        .{ .pc = 0x320, .opcode = .exp, .export_target = 0x20, .export_enable = 0xf, .src0 = vgpr(11), .src1 = vgpr(12), .src2 = float(0), .src3 = float(1), .src_count = 4 },
        .{ .pc = 0x328, .opcode = .exp, .export_target = 0x21, .export_enable = 0xf, .src0 = vgpr(4), .src1 = vgpr(5), .src2 = vgpr(6), .src3 = vgpr(7), .src_count = 4 },
        .{ .pc = 0x330, .opcode = .s_endpgm },
    });
    return rdna2.translateSpirv(allocator, &program, .{
        .stage = .vertex,
        .vertex_index_vgpr = 5,
...
fn buildUnityUiFragmentSpirv(
    allocator: std.mem.Allocator,
    binding: rdna2.spirv.SampledImageBinding,
    fragment_extent: [2]u32,
    single_channel_mask_component: ?u32,
) !rdna2.spirv.Module {
    const vgpr = struct {
        fn at(reg: u32) rdna2.Operand {
            return .{ .kind = .vgpr, .reg = reg };
    ...
    const sample_pc = binding.instruction_pc orelse 0x40;

    var program = rdna2.Program{ .code = &.{}, .instructions = .empty };
    defer program.deinit(allocator);
    try program.instructions.appendSlice(allocator, &.{
        .{ .pc = 0, .opcode = .v_interp_mov_f32, .dst = vgpr(0), .src0 = uint(0), .src1 = uint(0), .src2 = uint(0), .src_count = 3 },
        .{ .pc = 4, .opcode = .v_interp_mov_f32, .dst = vgpr(1), .src0 = uint(1), .src1 = uint(0), .src2 = uint(1), .src_count = 3 },
        .{ .pc = 8, .opcode = .v_interp_mov_f32, .dst = vgpr(4), .src0 = uint(0), .src1 = uint(1), .src2 = uint(0), .src_count = 3 },
        .{ .pc = 12, .opcode = .v_interp_mov_f32, .dst = vgpr(5), .src0 = uint(1), .src1 = uint(1), .src2 = uint(1), .src_count = 3 },
        .{ .pc = 16, .opcode = .v_interp_mov_f32, .dst = vgpr(6), .src0 = uint(2), .src1 = uint(1), .src2 = uint(2), .src_count = 3 },
        .{ .pc = 20, .opcode = .v_interp_mov_f32, .dst = vgpr(7), .src0 = uint(3), .src1 = uint(1), .src2 = uint(3), .src_count = 3 },
        .{ .pc = sample_pc, .opcode_id = 0x20, .opcode = .image_sample, .dst = vgpr(8), .src0 = vgpr(0), .src1 = sgpr(binding.resource_sgpr), .src2 = sgpr(binding.sampler_sgpr), .src_count = 3, .data_mask = 0xf, .image_dimension = .dim_2d, .image_address_components = 2 },
    });
    if (single_channel_mask_component) |coverage_component| {
        // Unity font atlases are R8 coverage textures. Vulkan supplies the
        // guest-selected channels through the image-view swizzle. Textures
        // commonly select {0,0,0,R}, so the physical R coverage arrives in A
        // rather than the sampled vector's first component. Preserve the
        // vertex colour and multiply alpha by whichever component selects R.
        try program.instructions.appendSlice(allocator, &.{
            .{ .pc = sample_pc + 8, .opcode = .v_mov_b32, .dst = vgpr(12), .src0 = vgpr(8 + coverage_component), .src_count = 1 },
            .{ .pc = sample_pc + 12, .opcode = .v_mov_b32, .dst = vgpr(8), .src0 = vgpr(4), .src_count = 1 },
            .{ .pc = sample_pc + 16, .opcode = .v_mov_b32, .dst = vgpr(9), .src0 = vgpr(5), .src_count = 1 },
            .{ .pc = sample_pc + 20, .opcode = .v_mov_b32, .dst = vgpr(10), .src0 = vgpr(6), .src_count = 1 },
            .{ .pc = sample_pc + 24, .opcode = .v_mul_f32, .dst = vgpr(11), .src0 = vgpr(7), .src1 = vgpr(12), .src_count = 2 },
        });
    } else {
        try program.instructions.appendSlice(allocator, &.{
            .{ .pc = sample_pc + 8, .opcode = .v_mul_f32, .dst = vgpr(8), .src0 = vgpr(8), .src1 = vgpr(4), .src_count = 2 },
            .{ .pc = sample_pc + 12, .opcode = .v_mul_f32, .dst = vgpr(9), .src0 = vgpr(9), .src1 = vgpr(5), .src_count = 2 },
            .{ .pc = sample_pc + 16, .opcode = .v_mul_f32, .dst = vgpr(10), .src0 = vgpr(10), .src1 = vgpr(6), .src_count = 2 },
            .{ .pc = sample_pc + 20, .opcode = .v_mul_f32, .dst = vgpr(11), .src0 = vgpr(11), .src1 = vgpr(7), .src_count = 2 },
        });
    }
    try program.instructions.appendSlice(allocator, &.{
        .{ .pc = sample_pc + 28, .opcode = .exp, .export_target = 0, .export_enable = 0xf, .export_done = true, .src0 = vgpr(8), .src1 = vgpr(9), .src2 = vgpr(10), .src3 = vgpr(11), .src_count = 4 },
        .{ .pc = sample_pc + 36, .opcode = .s_endpgm },
    });
...
fn buildUiProbeFragmentSpirv(
    ...
    defer program.deinit(allocator);
    try program.instructions.appendSlice(allocator, &.{
        // Diagnostic copy of the first sampled image using PARAM0.xy.
    ...
    for (0..4) |corner| {
        ...
        while (pair_index < 33) : (pair_index += 1) {
            ...
            if (corner < 3) {
                ...
                // The corner a rectangle list leaves out. Native RECT_LIST
                // packets omit the corner opposite v2 (v0 + v1 - v2). Merged
                // NGG TRI_LIST export packets used by fullscreen passes give
                // v0 as the shared corner and omit its opposite instead
                // (v1 + v2 - v0).
                ...
                const sum = next_id;
                next_id += 1;
                const first: usize = if (completion_mode == 2) 1 else 0;
                const second: usize = if (completion_mode == 2) 2 else 1;
                const opposite: usize = if (completion_mode == 2) 0 else 2;
                try appendSpirvInstruction(allocator, &body, 129, &.{
                    type_vector4,
                    sum,
                    loaded[first],
                    loaded[second],
                ...
                value = next_id;
                next_id += 1;
                try appendSpirvInstruction(allocator, &body, 131, &.{
                    type_vector4,
                    value,
                    sum,
                    loaded[opposite],
...
test "NGG LDS export inference accepts dynamic clip depth" {
    const reg = struct {
        fn make(kind: gpu.ShaderOperandKind, index: u32) gpu.ShaderOperand {
            return .{ .kind = kind, .reg = index };
        }
    }.make;
    var instructions = [_]gpu.ShaderInstruction{.{}} ** 3;
    instructions[0] = .{
...
test "R11G11B10 float color targets use the matching packed Vulkan format" {
    var descriptor = std.mem.zeroes(gpu.resources.ColorTarget);
    descriptor.format = 6;
    descriptor.number_type = 7;
...
test "R16 UNORM storage images use the matching typed Vulkan format" {
    const format = storageImageFormat(7).?;
    try std.testing.expectEqual(gpu.ShaderSpirvStorageImageFormat.r16_unorm, format.spirv);
    try std.testing.expectEqual(vk.format_r16_unorm, format.vulkan);
...
test "10-10-10-2 UNORM color targets use the matching packed Vulkan format" {
    ...
    const format = colorTargetFormat(descriptor).?;
    try std.testing.expectEqual(vk.format_a2b10g10r10_unorm_pack32, format.vulkan);
    try std.testing.expectEqual(@as(u8, 4), format.bytes_per_texel);
...
test "color target component swap maps exports and logical write masks" {
    ...
    descriptor.component_swap = 0;
    try std.testing.expectEqual(@as(u8, 0xe4), colorTargetExportMapping(descriptor));
    try std.testing.expectEqual(@as(u32, 0x1), mapColorWriteMask(0x1, colorTargetExportMapping(descriptor)));

    descriptor.component_swap = 1;
    try std.testing.expectEqual(@as(u8, 0xc6), colorTargetExportMapping(descriptor));
    try std.testing.expectEqual(@as(u32, 0x4), mapColorWriteMask(0x1, colorTargetExportMapping(descriptor)));
    try std.testing.expectEqual(@as(u32, 0x1), mapColorWriteMask(0x4, colorTargetExportMapping(descriptor)));

    descriptor.component_swap = 2;
    try std.testing.expectEqual(@as(u8, 0x1b), colorTargetExportMapping(descriptor));
    try std.testing.expectEqual(@as(u32, 0x8), mapColorWriteMask(0x1, colorTargetExportMapping(descriptor)));

    descriptor.component_swap = 3;
    try std.testing.expectEqual(@as(u8, 0x93), colorTargetExportMapping(descriptor));
}

test "packed UNORM target accepts component swap through the export mapping" {
    var descriptor = std.mem.zeroes(gpu.resources.ColorTarget);
    descriptor.format = 9;
    descriptor.number_type = 0;
    descriptor.component_swap = 1;
...
test "RGBA32F color targets use the matching Vulkan format" {
    var descriptor = std.mem.zeroes(gpu.resources.ColorTarget);
    descriptor.format = 14;
    descriptor.number_type = 7;

    const format = colorTargetFormat(descriptor).?;
    try std.testing.expectEqual(vk.format_r32g32b32a32_sfloat, format.vulkan);
    try std.testing.expectEqual(@as(u8, 16), format.bytes_per_texel);
...
test "ambiguous sampled DIM follows the descriptor image type" {
    ...
    try std.testing.expectEqual(
        rdna2.spirv.SampledImageDimension.two_d,
        sampledImageDimensionForInstruction(.dim_3d, .color_2d).?,
    );
}
...
test "MSAA color targets keep their sample count and drop FMASK" {
    ...
    try std.testing.expectEqual(@as(u16, 3), host.last_array_slice);
}
...
test "sampled image views honor sRGB and destination selectors" {
    ...
    try std.testing.expectEqual(vk.format_r32_sfloat, sampledImageFormat(22, false).?);
    try std.testing.expectEqual(vk.format_r32_sfloat, sampledImageFormat(22, true).?);
    try std.testing.expectEqual(@as(u8, 4), storageImageBytesPerTexel(22));
    ...
    try std.testing.expectEqual(vk.format_r32g32b32a32_sfloat, sampledImageFormat(77, false).?);
    try std.testing.expectEqual(vk.format_r32g32b32a32_sfloat, sampledImageFormat(77, true).?);
    try std.testing.expectEqual(@as(u8, 16), storageImageBytesPerTexel(77));
...
test "R11G11B10 float presentation conversion preserves RGB channels" {
    ...
    const blue_one: u32 = (15 << 5) << 22;
    const source_words = [_]u32{ 0, red_one, green_one, blue_one };
    var rgba: [16]u8 = undefined;
    convertR11G11B10ToRgba8(std.mem.asBytes(&source_words), &rgba);
...
test "NGG auto rectangle uses a triangle strip topology" {
    ...
    try std.testing.expectEqual(
        @as(u32, 0),
        rectangleCompletionMode(.export_shader, 4, .{ .index_count = 3 }, false),
    );
    try std.testing.expectEqual(
        @as(u32, 0),
        rectangleCompletionMode(.vertex, 4, .{ .index_count = 3 }, false),
    );
    try std.testing.expectEqual(
        @as(u32, 1),
        rectangleCompletionMode(.vertex, 17, .{ .vertex_count = 3 }, false),
    );
    try std.testing.expectEqual(
        @as(u32, 2),
        rectangleCompletionMode(.export_shader, 4, .{ .index_count = 3 }, true),
    );
    try std.testing.expectEqual(
        @as(u32, 0),
        rectangleCompletionMode(.vertex, 4, .{ .index_count = 3 }, true),
    );

    const ngg = expandRectangleListDraw(7, .{ .vertex_count = 3, .first_vertex = 9 });
    try std.testing.expectEqual(@as(u32, 4), ngg.vertex_count);
    try std.testing.expectEqual(@as(u32, 9), ngg.first_vertex);
    const legacy = expandRectangleListDraw(17, .{ .vertex_count = 3 });
    try std.testing.expectEqual(@as(u32, 4), legacy.vertex_count);
    const indexed = expandRectangleListDraw(7, .{ .vertex_count = 3, .index_count = 3 });
    try std.testing.expectEqual(@as(u32, 3), indexed.vertex_count);
}

test "direct indexed draw honors persistent index type" {
    const body = [_]u32{ 0xffff, 0x1234_5000, 0x20, 6, 0 };
    const packet = gpu.pm4.Packet{
        .kind = .command,
        .opcode = gpu.pm4.draw_index_2,
        .header = 0,
        .body = &body,
    };
    var state = gpu.State{};
    state.instance_count = 5;
    try state.writeRegister(.uconfig, 0x24a, @bitCast(@as(i32, -7)));

    const u16_draw = Renderer.decodeDirectGuestDraw(packet, &state).?;
    try std.testing.expect(!u16_draw.index_uint32);
    try std.testing.expectEqual(@as(u32, 5), u16_draw.instance_count);
    try std.testing.expectEqual(@as(i32, -7), u16_draw.vertex_offset);
    try std.testing.expectEqual(@as(u64, 0x20_1234_5000), u16_draw.index_address);
    try std.testing.expectEqual(@as(?u32, 6), u16_draw.index_count);

    state.index_type = 1;
    const u32_draw = Renderer.decodeDirectGuestDraw(packet, &state).?;
    try std.testing.expect(u32_draw.index_uint32);
}
...
test "frame upload ring aligns snapshots and rejects overflow" {
    var cursor: usize = 0;
    try std.testing.expectEqual(@as(?usize, 0), reserveAlignedRange(&cursor, 1024, 17, 256));
    try std.testing.expectEqual(@as(usize, 17), cursor);
    try std.testing.expectEqual(@as(?usize, 256), reserveAlignedRange(&cursor, 1024, 32, 256));
    try std.testing.expectEqual(@as(usize, 288), cursor);
    try std.testing.expectEqual(@as(?usize, null), reserveAlignedRange(&cursor, 300, 32, 256));
    try std.testing.expectEqual(@as(usize, 288), cursor);
}
